...
**In progress:** live verification pass (step 3 webhook via ngrok waits on ngrok install + dashboard webhook creation).
**Next:** trigger a failing test payment on the subscription's short_url, confirm the signed webhook -> audit row in uvicorn logs; final commit.

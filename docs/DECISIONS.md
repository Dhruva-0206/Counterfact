...
## ADR-015: What "retry" means on Razorpay's Subscriptions product (live executor)
**Context.** Live test-mode run against a real subscription (`sub_TXXDsVmg4d3fkR`): `subscription.fetch` and `invoice.all` succeed; `payment.createRecurring` fails with `BadRequestError: The id provided does not exist` because a Subscriptions-product subscription carries no customer/token until the customer authenticates it, and Razorpay's docs list no merchant-initiated charge or retry endpoint for Subscriptions (Razorpay retries a `pending` subscription itself; a `halted` one is paid by the customer via the invoice link). `invoice.notify_by` is rejected on subscription-generated invoices without a customer contact (`Operation not allowed for Invoice in issued status`).
**Decision.** `RazorpayExecutor` maps arms to the endpoints that exist: escalation -> `subscription.fetch` snapshot; retry arms -> `payment.createRecurring` only when the event carries a saved token, customer and order (Recurring Payments product), otherwise `invoice.all(subscription_id)` to find the outstanding invoice, record its pay link and Razorpay's own schedule, and mark the action `deferred`; reminder -> `invoice.notify_by`, and when Razorpay refuses, `deferred` with the exact error and the pay link so the merchant sends it through its own channel. Nothing is faked as executed; no outstanding invoice -> `skipped` with the reason.
**Consequences.** On Subscriptions the merchant-controlled arms are reminder, escalation and no-action; retry *timing* control requires the tokenised Recurring Payments product, which the executor supports when a token is present. This is stated in `docs/ARCHITECTURE.md` and the README limitations.

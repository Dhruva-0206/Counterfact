...
## ADR-014: The `drifted` variant answers "why ML" with evidence
**Context.** Under `calibrated` the ML policy ties the rule table on rupees. The rule table is oracle-informed (written with the simulator's true per-category probabilities), so a tie is not a weakness of the learner, but framing is not evidence.
**Decision.** A fourth simulator variant, `drifted`: the calibrated world except that two merchants depart from the taxonomy. FitPulse: `bank_technical` failures behave like insufficient funds (liquidity dynamics, no outage clock) and reminders about insufficient funds backfire (lift below one, 4x churn). ScaleOps: insufficient-funds retries only succeed on the day-7 AP cycle (payday irrelevant), and failed mandates need a human (retries fail, escalation resolves 70%). Merchant id is already a feature, so the T-learner can learn the interaction; a category rule table cannot. No re-calibration (Razorpay default lands at 58.5% on the holdout without tuning).
**Consequences.** ML beats the rule table by 32% on rupees under drift (Rs 818k vs Rs 617k per 1,000) and by 57% on the drifted B2B merchant, with less than a third of the reminders; under `calibrated` it ties. The pitch line "matches a rule table written with perfect knowledge of the taxonomy, using 63-83% fewer reminders, abstains when nothing works, and beats it when merchants diverge from the taxonomy" is backed by `make eval` on all four variants.

## ADR-015: Two executor modes on Razorpay (tokenised recurring vs Subscriptions)
**Context.** Live test-mode runs (2026-09-03): on a Subscriptions-product subscription there is no merchant-initiated charge or retry endpoint (Razorpay retries `pending` itself; `halted` is paid by the customer via the invoice link), `invoice.notify_by` is refused on subscription-generated invoices without a customer contact (`Operation not allowed for Invoice in issued status`), and `payment.createRecurring` needs a registered token. With the authenticated subscription's token (`token_TXXXCyI5Tx3WDS`, customer `cust_SyobLyHEnrJxSn`) and a fresh order, both `POST /v1/payments/create/recurring` and the S2S `/payments/create/json` return `BadRequestError: The requested URL was not found on the server`, while `order.create` succeeds: Recurring Payments is not enabled on this test account.
**Decision.** `RazorpayExecutor` exposes two explicit modes. (1) *Tokenised recurring*: the agent executes the charge directly with idempotency: `order.create(receipt = idempotency_key, payment_capture = 1)`, a pre-charge lookup of orders carrying that receipt (an existing captured/authorized payment is reused, never re-charged), then `payment.createRecurring`; the fault injector for the demo raises a transport error inside the real SDK call path so backoff, queueing and re-drive are exercised on the live endpoint; `scripts/verify_charges.py` checks Razorpay's own `order.payments` and `payment.all` for exactly one payment per executed key and none per failed/queued key. (2) *Razorpay Subscriptions*: the agent controls timing, outreach and escalation and records Razorpay's schedule; retry arms are `deferred` with the outstanding invoice's pay link, reminders `deferred` with Razorpay's exact refusal when `notify_by` is not allowed, escalation snapshots the subscription, and a paid-up subscription yields `skipped`. Nothing is reported as `executed` unless a provider call succeeded.
**Consequences.** Mode 1 is complete and tested up to the charge call; executing a real charge on this account needs Recurring Payments enabled by Razorpay (dashboard/support), after which `make demo` with `--tokenized-events` will show real `createRecurring` ledger rows next to deferred subscription rows. The demo batch and the docs never present deferred or failed rows as executed.

## ADR-016: Explanation validator allows contrastive mentions, rejects recommendations
**Context.** First live Claude run: 9 of 10 valid explanations were rejected for phrases like "rather than retrying immediately", and one accepted text called the uplift a "success probability".
**Decision.** The prompt passes the exact action phrase and labels the numbers as incremental recovery probability over doing nothing; the validator requires the chosen action, rejects out-of-set actions, and rejects another arm only when it is mentioned without a contrast marker in the same sentence. Explanations are cached in the audit store and preserved on event replay (the SQLite upsert keeps existing explanations and outcomes).
**Consequences.** 10/10 live explanations pass; a second run on the same rows makes zero API calls.

## ADR-017: Explanations must state the baseline and avoid certainty language
**Context.** Live explanations were accurate but two read as certain ("loss of Rs 999", "necessary") on rows with a 9-12% chance of recovering with no action, and several omitted the baseline.
**Decision.** The validator requires the no-action recovery probability to appear as the exact percentage next to a no-action phrase, and rejects certainty language ("loss of Rs X", "necessary", "will be lost", "guaranteed", "certain", "permanent loss" without a probability) whenever that baseline exceeds 5%. The prompt asks for the baseline sentence explicitly. Cached explanations are re-validated (`revalidate`) and only failing rows are regenerated (`reexplain`).
**Consequences.** 6 of 10 cached rows were regenerated once and all 10 pass; explanations now read as probabilities, which is the product's whole point.

## ADR-018: Payment Links are the live execution path; createRecurring stays as the tokenised path
**Context.** Recurring Payments is not enabled on the test account (ADR-015: 404 on `POST /v1/payments/create/recurring`), and Subscriptions offers no merchant retry. Payment Links work on every Razorpay account.
**Decision.** `remind_and_retry` on a subscription event executes a Razorpay Payment Link: amount = outstanding invoice amount (else the failed payment's amount), `reference_id = idempotency_key`, customer contact from the subscription's customer, `notify` SMS/email by Razorpay, `reminder_enable`, `expire_by` 7 days. Before creating, links are looked up by `reference_id`, so a re-drive after a lost response reuses the link. The ledger records `plink_id` and `short_url` as `executed`. `payment_link.paid` on the webhook maps `reference_id` back to the decision and records the recovery outcome. The transport-level fault injector covers `/payment_links`, and `scripts/verify_charges.py` counts links per reference on Razorpay's side. Retry-timing arms on Subscriptions remain `deferred` (never "executed"); the tokenised `createRecurring` path stays in code and tests for accounts with Recurring Payments enabled.
**Consequences.** The "one failure handled gracefully" moment runs on a live endpoint with Razorpay-side evidence of exactly one link per key. The reminder arm's real cost includes a Payment Link fee in production; the demo's message cost parameter stands in for it.

## ADR-019: cloudflared quick tunnel, not ngrok, for the webhook demo
**Context.** Razorpay's dashboard needs a public HTTPS URL to deliver test-mode webhooks to the local API server. ngrok was the first choice and failed twice on this machine: the installed agent (3.3.1) is below the account minimum (3.20.0, `ERR_NGROK_121`), and the 3.39.11 binary that `ngrok update` fetched is blocked from executing by Windows Smart App Control ("An Application Control policy has blocked this file. Malicious binary reputation"); `winget upgrade ngrok.ngrok` reports nothing newer. Unblocking it means relaxing a machine-wide Windows security policy, which is a bad trade for a demo tunnel.
**Decision.** Use `cloudflared tunnel --url http://localhost:8000` (`winget install Cloudflare.cloudflared`). It needs no account, no authtoken and no signup, and the MSI is signed and passes Smart App Control. The tunnel is verified before the dashboard is touched: `GET /health` through the public host, then `scripts/webhook_selftest.py --url https://<host>/webhook/payment_failed`, which must show 401 on a tampered signature and 200 plus an audit row on a signed body. `docs/WEBHOOK.md` documents cloudflared as the project's tunnel and ngrok as an alternative.
**Consequences.** The public webhook path is provable end to end without Razorpay and without changing any security setting. Like ngrok's free plan, a quick tunnel gets a new hostname on every restart, so the dashboard webhook URL is re-pasted per session; that is the one manual step left in the live demo.

## ADR-020: The webhook server's audit store is selectable, and a link only reconciles within its own store
**Context.** During live verification a paid Payment Link recorded no outcome. `payment_link.paid` maps a link back to its decision through `reference_id` = the decision's idempotency key, and `find_by_key` searches the store the server was built with. The paid link had been created by an earlier batch writing `reports/audit/live_demo`, while the server read `reports/audit/api`, so the handler correctly returned `{"ignored": "payment_link.paid", "reason": "no decision with this reference_id"}`. The mapping was right; the ledger was the wrong one.
**Decision.** Keep the strict behaviour: never guess across stores, never invent a decision for an unknown reference, and keep answering with an explicit `ignored` plus a reason rather than a silent 200. Make the store selectable instead: `COUNTERFACT_API_AUDIT_DIR` (else `reports/audit/api`), with `build_agent(audit_dir=...)` still winning for tests. `docs/WEBHOOK.md` states the rule.
**Consequences.** Links the server creates itself always reconcile, which is the production shape: one long-lived store behind the webhook. Reconciling links made by an offline batch is a matter of pointing the server at that batch's store. A cross-store lookup would have made outcome attribution non-deterministic and is not worth it for a demo convenience.

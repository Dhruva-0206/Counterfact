...
> Status: Phases 1-4 complete plus the `drifted` variant; dashboard and demo script next. Every number in this README is regenerated by `make data && make train && make eval` (plus `make dial`, `make sensitivity`, `make demo`).
...
make data      # 50k failures x 4 simulator variants
